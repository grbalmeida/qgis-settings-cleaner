...
- The plugin now does one thing: it clears all QGIS settings and closes QGIS. The action is **Clear All QGIS Settings and Close...**, in the **Plugins** menu and on the **Plugins** toolbar.
- User-facing texts avoid the term "user profile", which says little to most users, and speak of the QGIS settings and the folder they are kept in. They also say that every installation of the same QGIS version on the computer is affected, because they share that folder.
- The confirmation dialog says what is deleted (settings, saved passwords, plugins including this one, styles, bookmarks, Processing models), that it cannot be undone, and which folder will be emptied. **Cancel** is the default button; Enter no longer confirms the deletion.
- The current project is closed before anything is deleted, so QGIS asks about unsaved changes first and cancelling there keeps the settings. Before, they were already gone when QGIS asked, and cancelling left QGIS running without them. For the same reason the plugin refuses to run while background tasks are active.
...
- All messages are translated to Portuguese, with the words QGIS itself uses (complementos, favoritos espaciais), and the compiled `.qm` is versioned so a clone of the repository works as is.

### Added

- The first time the plugin loads, a message in the message bar says where to find it.
